...
.chat-bubble { display: inline-block; padding: 0.6rem 1rem; border-radius: 10px; font-size: 0.85rem; line-height: 1.6; max-width: 90%; white-space: pre-wrap; }
...
def render_step_bar(label: str, key: str, icon: str):
    css = step_status_class(st.session_state.pipeline_steps, key)
    st.markdown(f"""
...
with st.sidebar:
    ...
    if input_mode == "YouTube URL":
        source_text = st.text_input("YouTube URL or file path", placeholder="https://youtube.com/watch?v=...")
    else:
        uploaded_file = st.file_uploader("Upload audio/video", type=["mp4", "mov", "mkv", "mp3", "wav", "m4a"])
    ...
    if st.session_state.pipeline_done or st.session_state.processing:
        ...
        for step, icon, label in STEP_DEFS:
...
if run_btn:
    ...
    if input_mode == "YouTube URL" and source_text.strip():
        resolved_source = source_text.strip()
    elif input_mode == "Upload file" and uploaded_file is not None:
        resolved_source = save_uploaded_file(uploaded_file)

    if not resolved_source:
        st.error("Please enter a YouTube URL / file path, or upload a file first.")
    else:
        st.session_state.pipeline_done = False
        ...
        try:
            ...
            for k, _, _ in STEP_DEFS:
                if st.session_state.pipeline_steps.get(k) == "active":
                    st.session_state.pipeline_steps[k] = "error"
            progress_placeholder.empty()

if st.session_state.last_error and not st.session_state.result:
    st.markdown(f"""
    <div class="card" style="border-color: rgba(239,68,68,0.4)">
        <div class="card-title"><span class="badge badge-red">Error</span></div>
        <div class="card-content">{st.session_state.last_error}</div>
    </div>""", unsafe_allow_html=True)
...
if st.session_state.result:
    ...
    if send_btn and user_input.strip():
        try:
            ...
            answer = f"⚠️ I ran into an error answering that: {e}"
        st.session_state.chat_history.append({"role": "user", "content": user_input.strip()})
        st.session_state.chat_history.append({"role": "assistant", "content": answer})
        st.session_state.chat_input_key += 1  # forces the text_input to reset
        st.rerun()
